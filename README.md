...
Self-Balancing Robot based on Arduino Uno, with an extra features
...
Hardware:
Arduino UNO R3
MPU 6050 - 6-axis intertial measurement unit combining gyro and accelerometer.
L293D motor shield - mounted onto the UNO for motor speed and direction control.
SG90 servo and HC-SR04 combined to track both the angle at which the object's detected and the distance from it.
2x TT DC Geared motors and the wheels.
2x 18650 2200Mah 3.7v batteries

Material:
Each of the three levels use acylic(plexiglass) to make the frame sturdy.
As well as the supports on the sides, which are just tranparent for aesthetics.
...
- There are three PID algorithms, each serving an important role:
    1) Balancing around a neutral setpoint. This PID is scanning for changes in tilt every 10ms
    2) Dynamically adjusting this setpoint in order to prevent the robot from drifting to a side. This is achieved through counting corrections made on one side and comparing to the amount of corrections made on the opposite side within a 400ms time window.
